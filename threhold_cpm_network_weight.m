...
function threhold_cpm_network_weight(cpm_path_result,wkdir,op_dir,freq_thresh,edge_type)
...
% edge_type: 1:weight:mean 0:binary matrix
    ...
    if isequal(edge_type,1)
        % weight mask
        load(fullfile(wkdir,'cpm_predictor_variables_all_ppts.mat'),'cpm_predictors');
        n_sub = size(cpm_predictors,3);
  
        for n = 1:n_sub
            pos_mask_cpm_predictors(:,:,n) = cpm_predictors(:,:,n).* pos_edge_mask;
            neg_mask_cpm_predictors(:,:,n) = cpm_predictors(:,:,n).* neg_edge_mask;
        end
        pos_edge_weight = mean(pos_mask_cpm_predictors,3);
        neg_edge_weight = mean(neg_mask_cpm_predictors,3);
        
        save(pos_mask_file, 'pos_edge_weight', '-ascii');
        save(neg_mask_file, 'neg_edge_weight', '-ascii');
    else  
    ...
    glasser_info = readtable('Z:\NN_FU\gPPI_Glasser_yihe\Glasser_atlas\glasser358.txt');  % glasser358 atlas path
    % update yihe
...
%     pos_T_region = glasser_info(pos_id,:);
    ...
    %% 03 briannet viewer
    template = 'C:\software\BrainNetViewer_20191031\Data\SurfTemplate\BrainMesh_ICBM152.nv'; %BrainnetViewer SurfTemplate path

    % pos network setting
    pos_node_file = fullfile(op_dir,'pos_node.node');
    pos_edge_file = fullfile(op_dir,'pos_net.edge');
    pos_cfg = 'Z:\NN_FU\gPPI_Glasser_yihe\CPMresults\CPM_00_Plotscript\Connectviewer_cfg\pos_cfg.mat';  %cfg.mat path
    ...
    neg_cfg = 'Z:\NN_FU\gPPI_Glasser_yihe\CPMresults\CPM_00_Plotscript\Connectviewer_cfg\neg_cfg.mat';  %cfg.mat path
